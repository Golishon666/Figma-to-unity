...
FIGUNITY handles the core conversion rules: `TEXT` to `TextMeshProUGUI`, simple solid or stroked panels to editable FIGUNITY graphics, complex/vector/image/background nodes to exported PNG `RawImage`, `Button - ...` frames to `Button`, Figma meter groups with `Track`/`Fill`/`Handle` to passive or active `UnityEngine.UI.Slider`, `clipsContent` and mask nodes to Unity masks, Figma auto-layout hints to layout groups, and common toggle/input/dropdown/scroll/tab controls to UGUI/TMP components.
